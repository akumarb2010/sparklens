...
//    out.println(s"ac.executorMap.size(appExecutorCount) is ${appExecutorCount}")
...
//          out.println(s"executorCount is ${executorCount} for percent ${percent}")
          if (executorCount > 0) {
...
//    val instanceTypes: List[mutable.Map[String, String]] = List(t2smallMap, t2largeMap, m5largeMap, m4largeMap, c5largeMap, r4largeMap)
...
//        leastNumInstance = instanceType("numInstance")
    ...
    out.println(s"\n${Console.GREEN} The total optimized cost will be $$" + BigDecimal(optimizedTotalCost).setScale(2, BigDecimal.RoundingMode.HALF_UP).toDouble + " with " + optimizedExecutorCount + " " + optimizedInstanceType + s" instances${Console.RESET}")
    out.toString()
  }
